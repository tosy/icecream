...
1.遗留负载的分布组成
观察一个最简单的场景：当有效带宽产生波动之后（有效带宽恢复到正常水培），发送端收到报告，确认有包丢失（或因报告超时、丢失而确认数据丢失），开始重发：
...
可以观察到：
在不停的发送过程中，重发负载在逐步减小且比例越来越低，每一次新的实时负载都会一定比例混入遗留负载中。
...
转换为额外带宽资源与需求讨论。
	...
	2.如果额外带宽不能持续保持，不能高效消化遗留负载，但可以不断的降低其中最早的数据占比最终接近0。
	但jitterbuffer因延迟需求不可能过长，等待时间有限。一定概率出现jitterbuffer蜂窝化一直取不到完整数据，在超时后不停丢弃数去。
