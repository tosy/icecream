...
class MultimediaTimer {
 public:
  ...
  MultimediaTimer() : event_(::CreateEvent(nullptr, true, false, nullptr)) {
    //tosy test timer
    hTimer_ = CreateWaitableTimerExW(nullptr, nullptr,
  ...
  */
  bool StartOneShotTimer(TimeDelta delay) {
    RTC_DCHECK(hTimer_ != nullptr);
    LARGE_INTEGER lldelay_ms;
    lldelay_ms.QuadPart = delay.ns()/100;
    return ::SetWaitableTimerEx(hTimer_, &lldelay_ms, 0, nullptr, nullptr,
                                nullptr, 0);
  }
  ...
  */
  //tosy test timer
  HANDLE* event_for_wait() { return &hTimer_; }
...
void TaskQueueWin::ScheduleNextTimer() {
  ...
  if (!timer_.StartOneShotTimer(delay)) {
    timer_id_ = ::SetTimer(nullptr, 0, delay.ms(), nullptr);
  }
  /*
