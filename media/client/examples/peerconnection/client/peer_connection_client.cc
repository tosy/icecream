...
PeerConnectionClient::PeerConnectionClient()
    ...
    //tosy mythread
  if (!client_thread_.get()) {
    client_thread_ = rtc::Thread::CreateWithSocketServer();
...
void logThreadPointer(std::string str) {
  void* p = rtc::Thread::Current();
  char buffer[1024];
  snprintf(buffer, sizeof(buffer), " Thread:: %p", p);
  RTC_LOG(LS_INFO) << str << buffer;
}
...
bool PeerConnectionClient::SendToPeer(int peer_id, const std::string& message) {
  ...
  while (working.exchange(true)) {
    Sleep(1);
  }
  
  if (!client_thread_->IsCurrent()) {
    logThreadPointer("1111111111111111111111111111");
    client_thread_->PostTask([this]() { this->ConnectControlSocket(); });
  } else {
    logThreadPointer("2222222222222222222222222222");
...
void PeerConnectionClient::OnConnect(rtc::Socket* socket) {
  logThreadPointer("OnConnect");

  RTC_DCHECK(!onconnect_data_.empty());
  size_t sent = socket->Send(onconnect_data_.c_str(), onconnect_data_.length());
  RTC_DCHECK(sent == onconnect_data_.length());
  
  logThreadPointer(onconnect_data_);
  ...
  if (ReadIntoBuffer(socket, &notification_data_, &content_length)) {
    ...
    if (ok) {
      ...
      if (my_id_ == static_cast<int>(peer_id)) {
        ...
        logThreadPointer("OnHangingGetRead!!!!!!!");
