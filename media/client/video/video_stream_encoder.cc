...
void VideoStreamEncoder::ConfigureEncoder(VideoEncoderConfig config,
  ...
  encoder_queue_.PostTask([this, config = std::move(config),
    ...
    //tosy test bitrate
    RTC_LOG(LS_INFO) << " TOSY TEST VideoStreamEncoder::ConfigureEncoder pending_encoder_reconfiguration_ true";
    ...
    if (last_frame_info_) {
      ...
      //tosy test
      RTC_LOG(LS_INFO) << "TOSY test set bitrate VideoStreamEncoder::ConfigureEncoder ReconfigureEncoder";
      ReconfigureEncoder();
    } else {
...
void VideoStreamEncoder::ReconfigureEncoder() {
  ...
  if (rate_allocator_ && last_encoder_rate_settings_) {
    ...
    //tosy test
    RTC_LOG(LS_INFO)
        << "TOSY test set bitrate  VideoStreamEncoder::ReconfigureEncoder SetEncoderRates";
...
void VideoStreamEncoder::OnFrame(Timestamp post_time,
  ...
  if (frames_scheduled_for_processing == 1 && !cwnd_frame_drop) {
    ...
    RTC_LOG(LS_INFO) << "TOSY VideoStreamEncoder::OnFrame drop frame!!!!!!!!"
                     << frames_scheduled_for_processing << " "
                     << cwnd_frame_drop_interval_.has_value();
    if (cwnd_frame_drop) {
...
void VideoStreamEncoder::MaybeEncodeVideoFrame(const VideoFrame& video_frame,
  ...
  if (!last_frame_info_ || video_frame.width() != last_frame_info_->width ||
    ...
    RTC_LOG(LS_INFO) << " TOSY TEST VideoStreamEncoder::MaybeEncodeVideoFrame pending_encoder_reconfiguration_ true";
    pending_encoder_reconfiguration_ = true;
  ...
  if (pending_encoder_reconfiguration_) {
      //tosy test
    RTC_LOG(LS_INFO)
        << "TOSY test set bitrate VideoStreamEncoder::MaybeEncodeVideoFrame pending_encoder_reconfiguration_ ReconfigureEncoder";
    ReconfigureEncoder();
    ...
    if (last_encoder_rate_settings_) {
        //tosy test
      RTC_LOG(LS_INFO) << "TOSY test set bitrate VideoStreamEncoder::MaybeEncodeVideoFrame SetEncoderRates";
...
void VideoStreamEncoder::EncodeVideoFrame(const VideoFrame& video_frame,
  ...
  static int64_t testSumMs = 0;
  static int64_t testSumDiffPostBeforeMs = 0;
  auto testms = rtc::TimeMillis();
  auto diff_post = testms - time_when_posted_us/1000;

  const int32_t encode_status = encoder_->Encode(out_frame, &next_frame_types_);
  was_encode_called_since_last_initialization_ = true;
  
  int64_t diff = rtc::TimeMillis() - testms;
  testCount++;
  testSumMs += diff;
  testSumDiffPostBeforeMs += diff_post;
  if (testCount >= 120) {
    int codecCost = testSumMs / testCount;
    RTC_LOG(LS_INFO) << "TOSY test codec cost : " << codecCost
                     << "ms max codec fps : " << 1000 / codecCost
                     << " before encodec cost: "
                     << testSumDiffPostBeforeMs / testCount;
    testCount = 0;
    testSumMs = 0;
    testSumDiffPostBeforeMs = 0;
  }
...
//  int64_t cur = rtc::TimeMillis();
  ...
  // 0 ms
//  RTC_LOG(LS_INFO) << "TOSY VideoStreamEncoder::OnEncodedImage cost :: " << rtc::TimeMillis() - cur;
...
void VideoStreamEncoder::OnBitrateUpdated(DataRate target_bitrate,
  ...
  //tosy test
  RTC_LOG(LS_INFO) << "TOSY test set bitrate VideoStreamEncoder::OnBitrateUpdated SetEncoderRates";
